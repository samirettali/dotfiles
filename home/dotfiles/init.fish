...
function session_exists --argument session
    set found (tmux list-sessions -F "#{session_name}" | grep -c "^$session\$")
    echo $found
end
...
function t
    ...
    if test -n "$argv[1]"
        set found (session_exists $argv[1])
        if test $found -eq "1"
            echo "session $argv[1] exists"
            set target $argv[1]
        else
            set directory (zoxide query "$argv[1]")
            set target (basename $directory)
            set target (echo $target | tr -d ' ' | tr '[:upper:]' '[:lower:]')
        end
    else
        set directory (zoxide query -l | gum filter)
        set target (basename $directory)
        set target (echo $target | tr -d ' ' | tr '[:upper:]' '[:lower:]')
    end
    ...
    if test -n "$target"
        ...
        set found (session_exists $target)
        if test $found -eq "0"
            tmux new-session -s "$target" -c "$directory" -d
        end

        echo "attaching to $target in $directory"
        if test -n "$TMUX"
